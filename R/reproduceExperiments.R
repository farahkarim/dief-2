...
experiment1 <- function(traces, metrics) {
  ...
  print(paste0("throughput: ",metrics$throughput))
  metrics$invtfft <- with(metrics, 1/metrics$tfft)
  print(paste0("invtfft: ", metrics$invtfft))
  metrics$invtotaltime <- with(metrics, 1/metrics$totaltime)
  print(paste0("intotaltime: ",metrics$invtotaltime))
  # Obtain queries.
  queries <- unique(traces$test)
  print(paste0("queries: ",queries))
